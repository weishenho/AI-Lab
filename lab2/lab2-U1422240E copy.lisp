...
(and (<= 1 x) (<= x h) (<= 1 y) (<= y w) (null (find (cons x y) sollist))))
...
(progn 
(incf moves)
(setf sollist (append sollist (list(cons x y) )));; add knight's position to solution path
...
(decf moves)
(setf sollist (cdr (reverse sollist))) ;;remove last position on the solution path
)))
...
(setq h m) ;;the height of the board
(setq w n) ;;the width of the board
(setq totalmoves (* h w)) ;;the totalmoves possible and also the goal
(setq moves 0) ;;starting number of move is 0
;;There is a maximum of 8 distinct move a knight can take with one action from any position
;;These offsets can find the moves that are possible relative to the knight's position
...
(if (= totalmoves moves) (return-from knights-tour-backtracking sollist)  (print "Can't Find a Solution") ))
